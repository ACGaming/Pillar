...
public enum GeneratorType {
    ...
    private final BlockPosProvider provider;
    private final boolean findLowest;

    GeneratorType(boolean findLowest, BlockPosProvider provider) {
    ...
    private static BlockPos surfacePos(StructureSchema schema, Random random, World world, BlockPos xzPos) {
        ...
        if (pos.getX() == 0 || state.getBlock() instanceof BlockLiquid || !isInYBounds(schema, pos.getY())) return null;
    ...
    private static BlockPos getTopLiquidBlock(World world, BlockPos pos) {
        ...
        for (blockpos = new BlockPos(pos.getX(), chunk.getTopFilledSegment() + 16, pos.getZ()); blockpos.getY() >= 0; blockpos = blockpos1) {
            ...
            if (state.getBlock() instanceof BlockLiquid) break;
        }
